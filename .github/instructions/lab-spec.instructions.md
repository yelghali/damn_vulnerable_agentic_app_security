...
| V7 | **Insecure runtime** — public endpoints, no network isolation, no monitoring, verbose errors | **Private endpoints / VNet**, **Defender for Cloud** AI threat protection, **Monitor/Log Analytics** auditing, rate limiting, safe error handling |
...
| 6 | **Secure runtime, AI gateway & monitoring** — front models + MCP/tool endpoints with **Azure API Management (AI Gateway)** (central auth, token-based throttling, key vaulting, logging, caching), private endpoints, Defender for Cloud AI, Monitor/Log Analytics, safe errors | V7, V10 | LLM10, T4/T8 | 35m |
...
Each module: *Scenario → Exploit it → Why it's dangerous (OWASP/MS mapping) → Remediate (steps) → Verify → MS Learn references.* Per-module times are annotated in `workshop.md`; the Core track fits a **half day (~4 h)** and the Extended track makes it a **full day**.
